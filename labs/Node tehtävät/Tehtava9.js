...
app.get('/', function(req,res){
    res.sendFile('Tehtava9.html', {root: __dirname});
});
...
app.post('/update', async (req, res) => {
    try {
        ...
        const result = await pool.query('INSERT INTO event (Name, Type, Location_Location_id) VALUES (?,?,?)', [jsonObj.eventName,jsonObj.eventType,jsonObj.eventLocation,]);
        let insertedId = result.insertId;
        console.log(result);
        await pool.query('INSERT INTO Event_date (Date,Event_id) VALUES (?,?)', [jsonObj.eventDate,insertedId]);
